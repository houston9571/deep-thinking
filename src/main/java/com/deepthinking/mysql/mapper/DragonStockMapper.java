...
public interface DragonStockMapper extends BaseMapper<DragonStock> {
    ...
    @Select("SELECT a.partner_code, a.partner_name, c.trade_date, b.dept_code, b.dept_name , c.net_buy_amount AS partnerNetBuyAmount, c.buy_amount AS partnerBuyAmount, " +
            "c.total_buy_ratio AS partnerTotalBuyRatio, c.sell_amount AS partnerSellAmount, c.total_sell_ratio AS partnerTotalSellRatio, d.*, e.*  " +
    ...
    @Select("SELECT y.*, d.*, e.* " +
            ...
            " FROM ( " +
            "  SELECT a.partner_code, a.partner_name, c.stock_code, c.stock_name, c.trade_date, c.net_buy_amount, c.total_net_buy_ratio, c.buy_amount, c.sell_amount " +
            "  FROM org_partner a LEFT JOIN org_partner_dept b ON a.partner_code=b.partner_code LEFT JOIN dragon_stock_detail c ON b.dept_code=c.dept_code " +
            "  WHERE a.partner_code= #{partnerCode} " +
            "  ) x GROUP BY partner_code, partner_name, stock_code, stock_name, trade_date " +
            " ORDER BY trade_date DESC " +
            ") y " +
            "LEFT JOIN dragon_stock d on y.stock_code=d.stock_code AND y.trade_date=d.trade_date " +
            "LEFT JOIN stock_kline_daily e on y.stock_code=d.stock_code AND y.trade_date=e.trade_date " +
            "LIMIT 30")
    List<DragonDetailStockKline> queryDragonPartnerDetail(@Param("partnerCode") String partnerCode);
